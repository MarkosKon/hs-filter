...
const Group = ({
  group, onRequestClose, modalInitiator, bg, color,
}) => {
  const {
      ...
      <Flex flexWrap="wrap">
        {group.map(filter => (
          <Button
            ...
            onClick={() => {
              dispatch(
                add({ filterGroup: modalInitiator, thingToAdd: { ...filter, id: createId() } }),
              );
              onRequestClose();
            }}
          >
            <Card fontSize={[1, '18px']} p={3} boxShadow="large" bg={bg} color={color} m={1}>
